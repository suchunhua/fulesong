...
<div class="page-content">
    <form method="post" action="{:url('editgroupHandle')}" id="form">
        <input type="hidden" name="id" value="{$arc.id}">
        <table id="simple-table" class="table  table-bordered table-hover">
            <tbody>
                <tr>
                    <td class="cfgtable w100">组名称:　</td>
                    <td><input type="text" name="title" id="title" value="{$arc.title}">
                        <input type="submit" class="btn btn-primary" style="padding:2px 10px 1px 10px" value="确定">
                    </td>
                </tr>
            </tbody>
        </table>
