	...
	<div class="main-content-inner">
		...
		<div class="page-content">
			<div class="row maintop">
				<div class="col-xs-12">
                	<a href="{:url('nav/add')}" class="btn btn-success borderno"><i class="fa fa-plus"></i> 新增</a>
                	{if condition="$navlist"}
                	<button  class="btn btn-info borderno mt10" id="setrank"><i class="fa fa-sort"></i> 排序</button>
                	{/if}
                </div>
			</div>
			<form action="{:url('nav/setrank')}" method="post" id="form">
				<div class="row">
					<div class="col-xs-12">
								...
								<tr {if condition="$nav['level'] EQ 0"}class="one"{/if} {if condition="$nav['level'] EQ 1"}class="two"{/if} {if condition="$nav['level'] EQ 2"}class="three"{/if} {if condition="$nav['level'] EQ 3"}class="four"{/if}>
									...
									<td style="padding: 12px 0 0 10px;">
										<div class="hidden-sm hidden-xs btn-group">
											...
											<a class="btn btn-xs btn-info" href="{:url('nav/add',['id'=>$nav['id']])}">
												<i class="ace-icon fa fa-plus bigger-120"></i></a>
											<a class="btn btn-xs btn-warning" href="{:url('nav/edit',['id'=>$nav['id']])}">
