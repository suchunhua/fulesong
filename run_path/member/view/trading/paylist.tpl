                    ...
                    <div class="page-content" >
                        <div class="row" >
							...
							{volist name="orderlist" id="order"}
								<tr id="{$order.order}"> 
									<td colspan="3" style="height: 50px; background: #eee">
											订单号：{$order.order}   
											订单时间：{$order.ordertime|date="Y-m-d H:i:s",###}
											
									</td>
									<td style="background: #eee" class="center">
										{if condition="$order['status'] EQ 1"}<a class="btn btn-info btn-xs">待发货</a>{/if}
										{if condition="$order['status'] EQ 2"}<a class="btn btn-success btn-xs">查看物流</a>{/if}
									</td>
								</tr>
