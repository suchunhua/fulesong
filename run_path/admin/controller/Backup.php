...
class Backup extends Basic {
	...
	public function index($type = null) {
		...
		case 'index':
			...
			$datalist  = array_map('array_change_key_case', $datalist);
			break;
		default:
	...
	public function optimize($tables = null) {
		if ($tables) {
			...
			if ($list) {
				exit(json_encode(['status'=>1,'msg'=>"数据表'{$tables}'优化完成"]));
			} else {
				exit(json_encode(['status'=>0,'msg'=>"数据表'{$tables}'优化出错请重试！"]));
			}
		}
	}
	/**
	...
	public function export($tables = null, $id = null, $start = null) {
		if (IS_POST && !empty($tables) && is_array($tables)) {
			...
			//创建备份文件
			$Database = new \kykkk\Database($file, $config);
			...
			//备份指定表
			$Database = new \kykkk\Database(session('backup_file'), session('backup_config'));
	...
	public function import($time = 0, $part = null, $start = null) {
		if (is_numeric($time) && is_null($part) && is_null($start)) {
			...
			//获取备份文件信息
			$name  = date('Ymd-His', $time) . '-*.sql*';
			$path  = realpath('./extend/databack') . DIRECTORY_SEPARATOR . $name;
			$files = glob($path);
			$list  = array();
			...
			$db = new \kykkk\Database($list[$part], array('path' => realpath('./extend/databack') . DIRECTORY_SEPARATOR, 'compress' => $list[$part][2]));
