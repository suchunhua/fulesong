...
Namespace.register("KYKKK.DATA");
Namespace.register("KYKKK.CORE");
Namespace.register("KYKKK.MYFUN");
Namespace.register("KYKKK.MSG");
Namespace.register("KYKKK.FN");
Namespace.register("KYKKK.TEMP");
$(function() {
    KYKKK.CORE = {
        myfun:function(name, fun, cover){
            if (typeof name == 'string' && typeof fun == 'function') {
                if (cover == true || typeof KYKKK.MYFUN[name] != 'function') {
                    KYKKK.MYFUN[name] = fun;
                }
            }
        },
        ...
        do:function(p){
            if (1 == KYKKK.TEMP.submit_token) {
                KYKKK.MSG.alert('提交得太快啦！');
                return;
            }
            KYKKK.TEMP.submit_token = 1;
            ...
            $.ajax({
                ...
                success:function(res){
                    KYKKK.TEMP.submit_token = 0;
                    if (!res.code) {
                        x.close();
                        KYKKK.MSG.alert(res.msg);
                    }else{
                ...
                error:function(XMLHttpRequest, textStatus, errorThrown){
                    KYKKK.TEMP.submit_token = 0;
                    x.close();
                    KYKKK.CORE.error(XMLHttpRequest, textStatus, errorThrown);
                }
            });
        },
        submit:function(p){
